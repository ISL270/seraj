...
Future<void> _openFilterTagSelectionBottomSheet(DuaFilters filters, BuildContext context) async {
      ...
      children: [
          ...
          child: BlocBuilder<DuaBloc, DuaState>(
            builder: (context, state) {
              final tags = state.searchedTags.isNotEmpty
                  ? state.searchedTags
                  : context.read<DuaBloc>().getTags();
              if (state.searchedTags.isEmpty &&
                  context.read<DuaBloc>().tagSearchController.text.isNotBlank) {
                return Center(
                  child: Text(
        ...
        ApplyFilterButton(
          onPressed: widget.filters.tags.isEmpty
              ? null
              : () {
                  context.read<DuaBloc>().add(DuaFiltered(widget.filters));
                  context.pop();
                },
        ),
        // Gap(6.h),
...
class _SearchBarWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: BlocBuilder<DuaBloc, DuaState>(
        builder: (context, state) {
          final cubit = context.read<DuaBloc>();
          return TextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: state.searchedTags.isNotEmpty
                  ? IconButton(
                      ...
                      onPressed: () {
                        cubit.add(const DuaTagSearched(''));
                        cubit.tagSearchController.clear();
                      },
                    )
                  : null,
            ),
            onChanged: (value) => cubit.add(DuaTagSearched(value)),
          );
        },
      ),
    );
